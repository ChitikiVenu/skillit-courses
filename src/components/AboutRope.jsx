...
export default function AboutRope({ storyRef, sections }) {
  ...
  useEffect(() => {
    ...
    if (!story || !inner || !girl || !thread) return undefined;
    ...
    const place = () => {
      ...
      const maxY = Math.max(minY, storyH - GIRL_H - 24);
      const y = Math.max(minY, Math.min(maxY, (window.innerHeight * 0.5 - top) / k - GIRL_H / 2));
  ...
  return (
    <div className={`about-lane${landed ? ' is-landed' : ''}`}>
