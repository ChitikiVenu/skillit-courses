...
// Lightweight per-programme facts (route, short name, headline duration) for the header and the router, so
// neither has to load the five full course files (~180KB) in the main bundle. Generated, so it can't drift.
const programmesLite = [cyberSecurity, aiMl, dataScience, socAnalyst, dataAnalyst].map((c) => ({
  key: c.routeBase.replace(/^\//, ''),
  routeBase: c.routeBase,
  name: c.COPY.courseShortName,
  duration: c.COPY.heroStats[0].value,
}));
writeFileSync(
  resolve(__dirname, '../src/data/programmesLite.js'),
  `// GENERATED by scripts/generate-sitemap.mjs (\`npm run sitemap\` / \`npm run build\`) — do not hand-edit.\n` +
    ...
    `export const PROGRAMMES_LITE = ${JSON.stringify(programmesLite, null, 2)};\n`,
);
console.log(`✓ Wrote src/data/programmesLite.js (${programmesLite.length} programmes)`);
...
const roleGroupsLite = ROLE_GROUPS.map((g) => ({
  ...
  label: g.label,
  href: g.href,
  roles: g.roles.map((r) => ({ href: r.href, title: r.title, skills: (r.skills ?? []).slice(0, 3) })),
}));
...
// Lightweight {routeBase: {slug, question}} of each programme's first blog post, for ModuleFlow.jsx's
// "How can I become a ...?" tablet. It used to import blogPostForCourse from blogPosts.js, which drags
// every article body (~1.2MB) into every course page just to read one title and slug.
const blogPrimary = {};
for (const p of BLOG_POSTS) {
  if (!blogPrimary[p.course.routeBase]) blogPrimary[p.course.routeBase] = { slug: p.slug, question: p.question };
}
writeFileSync(
  resolve(__dirname, '../src/data/blogPrimaryLite.js'),
  `// GENERATED by scripts/generate-sitemap.mjs (\`npm run sitemap\` / \`npm run build\`) — do not hand-edit.\n` +
    `// Each programme's first blog post (slug + question), so course pages need not load blogPosts.js.\n` +
    `export const BLOG_PRIMARY_LITE = ${JSON.stringify(blogPrimary, null, 2)};\n`,
);
console.log(`✓ Wrote src/data/blogPrimaryLite.js (${Object.keys(blogPrimary).length} programmes)`);
